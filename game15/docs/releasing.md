...
```bash
bash /home/radgh/claude/release.sh game15     # build + bundle into milestone_N/
...
2. `optimize_audio.sh game15` — Ogg-transcode any new audio (idempotent)
...
4. **Featured screenshot** — if `release_server/screenshots/game15.jpg` is missing or stale, run `take_screenshots.sh game15` or copy a hero image from the game's assets.
