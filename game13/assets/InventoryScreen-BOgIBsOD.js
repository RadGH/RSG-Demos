import{i as U,c as P,j as K,G as w,p as W,k as X,s as G,t as V,h as D,v as Y,w as J,y as Q,z as Z,B as O,D as tt,E as B,F as et,H as R,I as A,J as F,r as $,K as ot,L as st,M as it,N as at,O as nt}from"./play-BWvgRpqe.js";import"./modulepreload-polyfill-B5Qt9EMX.js";import"./savesClient-wbDfy9Yw.js";import"./version-BKBHz2yn.js";const rt=["weapon","offhand","head","chest","legs","hands","feet","ring1","ring2","necklace"],lt={weapon:"Weapon",offhand:"Off-hand",head:"Head",chest:"Chest",legs:"Legs",hands:"Hands",feet:"Feet",ring1:"Ring",ring2:"Ring",necklace:"Necklace"};function ct(C,e,t){var h;const u=[...e.inventory||[]];let n=0;for(const a of u){if((h=e.isManuallyUnequipped)!=null&&h.call(e,a.id))continue;const g=C.autoEquip;C.autoEquip=!0;try{const i=e.tryAutoEquip(a);i&&i.member&&i.member.id===C.id&&n++}catch{}C.autoEquip=g}n>0&&t&&t.playSfx("purchase")}class ft{constructor(e,t){this.manager=e,this.audio=t,this._el=null,this._selectedCharIdx=0,this._charScrollPos=new Map,this._tt=null,this._compareMode=!1,this._compareSecondary=!1,this._currentTooltipItem=null,this._isTouch=typeof window<"u"&&("ontouchstart"in window||navigator.maxTouchPoints>0)}onEnter(){this._build()}_build(){U("inv-styles",dt),this._el=P("div","inv-screen"),this.manager.uiOverlay.appendChild(this._el),this._render()}_render(){K(this._el,()=>this._renderImpl())}_renderImpl(){const e=w.get(),u=(e.inventoryContext||"default")==="party-inactive",n=e.bench||[],h=u?[...e.party,...e.companions,...n]:[...e.party,...e.companions];if(e.inventoryFocusId){const i=h.findIndex(m=>m.id===e.inventoryFocusId);i>=0&&(this._selectedCharIdx=i),e.inventoryFocusId=null}const a=h[this._selectedCharIdx]||h[0];this._selectedCharIdx>=h.length&&(this._selectedCharIdx=0),this._el.innerHTML=`
      <div class="inv-header">
        <div class="inv-char-tabs" id="char-tabs">
          ${h.map((i,m)=>{const f=n.includes(i);return`
      ...
      <div class="inv-layout">
        ...
        <div class="equip-panel">
          <div class="inv-char-header">
            ...
            <div class="inv-char-identity">
              ...
              ${a&&!(a.isCompanion&&a.class==="companion")?`
                <button type="button" class="auto-toggle${a.autoEquip?" on":""}" id="inv-autoequip" aria-pressed="${a.autoEquip?"true":"false"}" title="When new items appear in your bag and they're an upgrade for this character, auto-equip them. Items you manually unequip are remembered and never auto-equipped." style="margin-top:0.35rem">
                  ${a.autoEquip?'<span class="auto-check" aria-hidden="true">✓</span>':""}Auto Equip
                </button>
              `:""}
            </div>
          </div>
          ...
          <div class="equip-slots" id="equip-slots">
            ${(()=>{var f,x;const i=(a==null?void 0:a.isCompanion)&&(a==null?void 0:a.class)==="companion",m=(x=(f=a==null?void 0:a.equipment)==null?void 0:f.weapon)==null?void 0:x.twoHanded;return rt.map(s=>{var r;const o=(r=a==null?void 0:a.equipment)==null?void 0:r[s];return`
    ...
    `,this._wireEvents(),G(this._el);const g=this._el.querySelector("#stats-base-chk");g&&g.addEventListener("click",()=>{var s,o;const i=(s=this._el)==null?void 0:s.querySelector(".equip-panel"),m=i?i.scrollTop:0,f=!g.classList.contains("on");V(f),this.audio.playSfx("click"),this._render();const x=(o=this._el)==null?void 0:o.querySelector(".equip-panel");x&&(x.scrollTop=m)})}_slotsForItem(e,t){if(!t||!e)return[];const u=[];return t.type==="weapon"?(u.push("weapon"),(t.offHandOk||!t.twoHanded)&&!t.twoHanded&&u.push("offhand"),u):t.subtype==="ring"||t.slot==="ring"?["ring1","ring2"]:t.slot?[t.slot]:t.subtype?[t.subtype]:[]}_upgradeTier(e,t){var g;if(!e||!t||e.isCompanion&&e.class==="companion")return null;const u=this._slotsForItem(e,t);if(!u.length)return null;const n=e.equipment||{};for(const i of u)if(!n[i]){if(i==="offhand"&&((g=n.weapon)!=null&&g.twoHanded))continue;return"empty"}const h=D(t,e).total;let a=-1/0;for(const i of u){const m=n[i];if(!m)continue;const f=D(m,e).total;if(f<=0)continue;const x=(h-f)/f;x>a&&(a=x)}return a<=0||a===-1/0?null:a<=.05?"minor":a<=.2?"medium":a<=.5?"major":"huge"}_vsItemForCompare(e,t){if(!e||!t)return null;const u=e.equipment||{},n=this._slotsForItem(e,t);if(!n.length)return null;let h=n[0],a=n[1];n.includes("ring1")&&n.includes("ring2")&&(h="ring1",a="ring2");const g=this._compareSecondary&&a?a:h;return{vs:u[g]||null,slot:g,hasSecondary:!!a&&a!==h}}_renderCharStats(e){if(!e)return'<div class="stat-row"><span>No character selected</span></div>';const t=e.baseAttrs||e.attrs,u=e.attrs,n=Y(),h=e.equipment||{};let a=0;for(const p of Object.values(h))p!=null&&p.armor&&(a+=p.armor);const g=0,i=J(e);a+=i.armor||0;const m=Q(h),f=n?t:{STR:u.STR+(i.str||0),DEX:u.DEX+(i.dex||0),INT:u.INT+(i.int||0),CON:u.CON+(i.con||0)},x=Z(h)+(i.dmg||0),s=O(f,n?0:x,m),o=O(t,0,m),d=m==="magic"?"Magic Damage":m==="light"?"Light Damage":"Heavy Damage",r=n?0:i.magicResist||0,l=(p,y)=>({hp:y?st(e):50+p.CON*10,mp:y?ot(e):30+p.INT*8,hit:Math.min(95,70+Math.round(p.DEX*1.2)+(y&&i.hit||0)),dodge:Math.min(40,5+Math.round(p.DEX*.8)+(y&&i.dodge||0)),spl:p.INT*.025+(y&&i.spellPower||0)}),c=l(n?t:f,!n),b=l(t,!1),v=n?g:a,k=n?0:tt(e).resistAll||0,q=B(v,k),M=B(g,0),_=(p,y,E)=>{const z=it(y,E,p);return z?` style="color:${z}"`:""},H=["STR","DEX","INT","CON"].map(p=>{const y=p.toLowerCase(),E=f[p];return`<div class="stat-row"><span class="sr-label stat-label" data-stat="${p}">${p}</span><span class="sr-val"${_(y,E,t[p])}>${Math.floor(E)}</span></div>`}).join(""),T=new Set(["str","dex","int","con","hp","mp","dmg","armor","hit","dodge","magicresist","magicResist","spellpower","spellPower"]),L={goldFind:"Gold Find",xpFind:"XP Find",manaRegen:"Mana Regen",lifeSteal:"Life Steal",manaSteal:"Mana Steal",initiative:"Initiative",critChance:"Crit Chance",critDamage:"Crit Damage",spellPower:"Spell Power",tradePrices:"Trade Prices"},S=[];if(!n){try{const p=et(e);if((p==null?void 0:p.blockChance)>0){const y=`+${R(p.blockChance)}`;S.push(`<div class="stat-row"><span class="sr-label stat-label" data-stat="Block Chance">Block Chance</span><span class="sr-val" style="color:#6db3ff">${y}</span></div>`)}(p==null?void 0:p.blockPower)>0&&S.push(`<div class="stat-row"><span class="sr-label stat-label" data-stat="Block Power">Block Power</span><span class="sr-val" style="color:#6db3ff">+${Math.round(p.blockPower)}</span></div>`)}catch{}for(const p of Object.keys(i)){if(T.has(p)||T.has(p.toLowerCase()))continue;const y=i[p];if(!y)continue;const E=L[p]||p.replace(/([A-Z])/g," $1").replace(/^./,j=>j.toUpperCase()),N=(p==="goldFind"||p==="xpFind"||p==="critChance"||p==="critDamage"||p==="tradePrices")&&Math.abs(y)<=3?`+${R(y)}`:p==="lifeSteal"||p==="manaSteal"?`+${A(y,"pct")}`:`+${A(y,"auto")}`;S.push(`<div class="stat-row"><span class="sr-label stat-label" data-stat="${E}">${E}</span><span class="sr-val" style="color:#6db3ff">${N}</span></div>`)}}const I=S.length?S.join(""):'<div class="stat-row"><span class="sr-label" style="color:#5a4a42;font-style:italic">None</span><span class="sr-val" style="color:#5a4a42">—</span></div>';return`
      ...
      <div class="spo-box">
        ...
        <div class="spo-actions">
          ...
          <button type="button" class="spo-btn" id="spo-offhand" ${g?'disabled title="Unequip 2H weapon first"':""}>
            Off Hand${a?`<br><small style="color:#8a7a6a">Replaces: ${a.name}</small>`:""}${g?'<br><small style="color:#c04030">2H equipped</small>':""}
          </button>
        </div>
        <button type="button" class="spo-cancel" id="spo-cancel">Cancel</button>
      </div>
    `,n.querySelector("#spo-weapon").addEventListener("click",()=>{this.audio.playSfx("click"),this._doEquip(e,t,"weapon",u),$(n),this._render()}),n.querySelector("#spo-offhand").addEventListener("click",()=>{g||(this.audio.playSfx("click"),this._doEquip(e,t,"offhand",u),$(n),this._render())}),n.querySelector("#spo-cancel").addEventListener("click",()=>$(n)),this._el.appendChild(n)}_showRingPicker(e,t,u){var g,i;const n=P("div","slot-picker-overlay"),h=(g=e.equipment)==null?void 0:g.ring1,a=(i=e.equipment)==null?void 0:i.ring2;n.innerHTML=`
      <div class="spo-box">
