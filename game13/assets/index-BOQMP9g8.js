    ...
    `}_updateHud(){var t,a,s,i;for(const o of this._heroes){const r=(t=this._el)==null?void 0:t.querySelector(`#hp-${o.id}`),l=(a=this._el)==null?void 0:a.querySelector(`#mp-${o.id}`),c=(s=this._el)==null?void 0:s.querySelector(`#hv-${o.id}`);r&&(r.style.width=`${Math.max(0,o.hp/o.maxHp*100)}%`),l&&(l.style.width=`${Math.max(0,o.mp/o.maxMp*100)}%`),c&&(c.textContent=`${Math.max(0,o.hp)}/${o.maxHp} HP`)}const e=(i=this._el)==null?void 0:i.querySelector("#hud-round");e&&(e.textContent=this._round)}_log_(e,t="normal"){var i;this._log.push({msg:e,type:t});const a=(i=this._el)==null?void 0:i.querySelector("#cbt-log");if(!a)return;const s=w("div",`cbt-entry cbt-${t}`);for(s.textContent=e,a.appendChild(s);a.children.length>10;)a.removeChild(a.firstChild);a.scrollTop=a.scrollHeight}update(e){this._t+=e,this._particles=this._particles.filter(t=>(t.life-=e,t.x+=t.vx,t.y+=t.vy,t.vy+=50*e,t.life>0)),this._dmgNumbers=this._dmgNumbers.filter(t=>(t.life-=e,t.y-=38*e,t.life>0));for(const[t,a]of this._flashMap){const s=a-e;s<=0?this._flashMap.delete(t):this._flashMap.set(t,s)}if(this._phase==="START"){this._t>=this._startDelay&&(this._phase="PLAYING",this._t=0,this._log_(`⚔ ${this.encounter.name}`,"round"));return}this._phase==="PLAYING"&&(this._turnTimer+=e*(this._fastMode?2:1),!(this._turnTimer<Se)&&(this._turnTimer=0,this._executeTurn()))}_executeTurn(){var a;if(this._turnIdx>=this._turnOrder.length){this._round++,this._processStatusEffects(),this._buildTurnOrder(),this._log_(`── Round ${this._round} ──`,"round");return}const e=this._turnOrder[this._turnIdx++];if(!e.alive)return;if((a=e.statuses)==null?void 0:a.find(s=>s.type==="stun")){this._log_(`${e.name} is stunned and cannot act!`,"miss");return}e.skillCooldown>0&&e.skillCooldown--,e.isHero?this._heroAI(e):this._enemyAI(e),this._checkCombatEnd()}_heroAI(e){const a=m.get().party.find(d=>d.id===e.id),s=(a==null?void 0:a.class)||e.class,i=(a==null?void 0:a.level)||1,o=this._allEnemies.filter(d=>d.alive),r=this._allies.filter(d=>d.alive);if(!o.length)return;const c=se(s,i).filter(d=>d.type!=="passive").filter(d=>e.skillCooldown===0&&(d.mpCost||0)<=e.mp);let h=!1;if(c.length&&Math.random()<.45){let d=null;const p=c.find(g=>g.type==="heal"),v=r.find(g=>g.hp/g.maxHp<.5);p&&v&&(d=p),!d&&o.length>=3&&(d=c.find(g=>g.aoe==="group"||g.aoe==="row"||g.aoe==="all")),d||(d=c.find(g=>g.type==="melee"||g.type==="magic")),d||(d=c[0]),d&&(h=!0,e.mp-=d.mpCost||0,e.skillCooldown=2,this._executeSkill(e,d,o,r,a))}h||this._basicAttack(e,o,!0),this._updateHud()}_enemyAI(e){const t=this._allies.filter(a=>a.alive);t.length&&this._basicAttack(e,t,!1)}_basicAttack(e,t,a){t.sort((l,c)=>l.hp-c.hp);const s=t[0],i=Math.max(5,Math.min(95,e.hit-s.dodge));if(Math.random()*100>=i){this._log_(`${e.name} misses ${s.name}.`,"miss"),e.stance="attack",setTimeout(()=>{e.stance="ready"},300);return}const o=e.dmg[0]+Math.floor(Math.random()*(e.dmg[1]-e.dmg[0]+1)),r=Math.max(1,o-s.armor);this._applyDamage(e,s,r,a?"#ff8060":"#e8a020"),this._log_(`${e.name} → ${s.name}: ${r}`,a?"hero":"enemy"),e.stance="attack",setTimeout(()=>{e.stance="ready"},300),this._updateHud()}_executeSkill(e,t,a,s,i){var g,_,y,S;e.stance="spell",setTimeout(()=>{e.stance="ready"},400),this.audio.playSfx("spell");const o=(i==null?void 0:i.attrs)||{STR:8,DEX:8,INT:8,CON:8};if(t.type==="heal"){const f=[...s].sort((T,q)=>T.hp/T.maxHp-q.hp/q.maxHp)[0];if(!f)return;const C=Math.round((t.healMult||1.5)*(o.INT||8));f.hp=Math.min(f.maxHp,f.hp+C),this._spawnDmgNumber(f.x,f.y-50,`+${C}`,"#60e880"),this._log_(`${e.name} uses ${t.name}: heals ${f.name} for ${C}`,"hero");return}if(t.type==="buff"){this._log_(`${e.name} uses ${t.name}!`,"hero"),(g=t.effect)!=null&&g.dmgBuff&&t.target==="party"&&s.forEach(f=>{f.dmgBuff=(f.dmgBuff||0)+t.effect.dmgBuff,f.dmgBuffRounds=t.effect.duration||2}),(_=t.effect)!=null&&_.dmgReduct&&t.target==="self"&&(e.dmgReduct=t.effect.dmgReduct,e.dmgReductRounds=t.effect.duration||2);return}const r=this._getSkillStat(t.damageStat,o),l=t.damageMult||1;let c=e.dmg[0]+Math.floor(Math.random()*(e.dmg[1]-e.dmg[0]+1)),h=Math.round(c*l+r*.2),d=[];if(!t.aoe||t.aoe==="single")d=[a[0]];else if(t.aoe==="adjacent"){const f=(y=a[0])==null?void 0:y.groupIdx;d=a.filter(C=>C.groupIdx===f).slice(0,2)}else if(t.aoe==="group"){const f=(S=a[0])==null?void 0:S.groupIdx;d=a.filter(C=>C.groupIdx===f)}else t.aoe==="row"||t.aoe==="all"?d=a:d=[a[0]];const p=t.type==="magic"?"#c060ff":"#ff8060",v=[];for(const f of d){const C=Math.max(1,h-f.armor);this._applyDamage(e,f,C,p),v.push(f.name);for(const T of t.statusEffects||[])Math.random()<(T.chance||.5)&&this._applyStatus(f,T.type,T.duration||2,T.power||4)}this._log_(`${e.name} uses ${t.name}${d.length>1?` (×${d.length})`:""}: ${h} dmg`,"hero")}_getSkillStat(e,t){return!e||e==="str"?t.STR||8:e==="dex"?t.DEX||8:e==="int"?t.INT||8:e==="str_int"?Math.round(((t.STR||8)+(t.INT||8))/2):e==="str_dex"?Math.round(((t.STR||8)+(t.DEX||8))/2):t.STR||8}_applyDamage(e,t,a,s){let i=a;t.dmgReduct&&(i=Math.round(i*(1-t.dmgReduct))),t.hp-=i,this._flashMap.set(t.id,Ce),this._spawnParticles(t.x,t.y-30),this._spawnDmgNumber(t.x,t.y-50,i,s),this.audio.playSfx("hit"),t.hp<=0&&(t.hp=0,t.alive=!1,t.stance="death",this._log_(`${t.name} defeated!`,"death"))}_applyStatus(e,t,a,s){e.statuses||(e.statuses=[]);const i=e.statuses.find(r=>r.type===t);if(i){i.duration=Math.max(i.duration,a);return}e.statuses.push({type:t,duration:a,power:s});const o={burn:"🔥",poison:"☠",bleed:"🩸",stun:"⚡"};this._log_(`${e.name} is ${t}ed! ${o[t]||""}`,"death")}_processStatusEffects(){const e=[...this._allies,...this._allEnemies];for(const t of e)t.alive&&(t.statuses=(t.statuses||[]).filter(a=>{if(a.type==="burn"||a.type==="poison"||a.type==="bleed"){const s=Math.max(1,a.power||3);t.hp-=s,this._spawnDmgNumber(t.x,t.y-45,s,a.type==="burn"?"#ff6020":a.type==="poison"?"#60c020":"#c02020"),t.hp<=0&&(t.hp=0,t.alive=!1,t.stance="death",this._log_(`${t.name} perishes from ${a.type}!`,"death"))}return a.duration--,a.duration>0}),t.dmgBuffRounds>0&&(t.dmgBuffRounds--,t.dmgBuffRounds===0&&(t.dmgBuff=0)),t.dmgReductRounds>0&&(t.dmgReductRounds--,t.dmgReductRounds===0&&(t.dmgReduct=0)));this._updateHud()}_checkCombatEnd(){const e=this._allEnemies.every(a=>!a.alive),t=this._allies.every(a=>!a.alive);e&&this._phase==="PLAYING"?(this._phase="VICTORY",setTimeout(()=>this._victory(),800)):t&&this._phase==="PLAYING"&&(this._phase="DEFEAT",setTimeout(()=>this._defeat(),800))}_spawnDmgNumber(e,t,a,s){!e&&!t||this._dmgNumbers.push({x:e+(Math.random()-.5)*20,y:t,text:String(a),color:s,life:.9,maxLife:.9})}_spawnParticles(e,t){if(!e&&!t)return;const a=["#e8a020","#ff6040","#f0c060","#ff4040"];for(let s=0;s<7;s++)this._particles.push({x:e,y:t,vx:(Math.random()-.5)*100,vy:-(Math.random()*80+30),size:Math.random()*4+2,color:a[Math.floor(Math.random()*a.length)],life:Math.random()*.4+.15})}_victory(){this.audio.playSfx("victory");let e=0,t=0;const a=[];for(const h of this._allEnemies)if(e+=h.xpValue,t+=h.gold[0]+Math.floor(Math.random()*(h.gold[1]-h.gold[0]+1)),Math.random()<.15){const d=["sword","dagger","light_chest","ring"],p=M(d[Math.floor(Math.random()*d.length)],"magic","medium");p&&(a.push(p),m.addToInventory(p))}m.addGold(t);const s=this._allEnemies.length+(this.encounter._bossNodeId?10:0);m.addFame(s);const i=m.get();this._heroes.forEach((h,d)=>{i.party[d]&&(i.party[d].hp=h.hp)});const o=ve(i.party,e),r={border_roads:"thornwood",thornwood:"dust_roads",dust_roads:"ember_plateau",ember_plateau:"hell_breach",hell_breach:"shattered_core",shattered_core:null};let l=null;if(this.encounter._bossNodeId){i.completedBosses||(i.completedBosses=[]),i.completedBosses.push(this.encounter._bossNodeId);const h=r[this.encounter._zoneId];h&&!(i.unlockedZones||[]).includes(h)&&(i.unlockedZones||(i.unlockedZones=["border_roads"]),i.unlockedZones.push(h),l=`${{thornwood:"Thornwood Forest",dust_roads:"The Dust Roads (Act 2)",ember_plateau:"The Ember Plateau",hell_breach:"The Hell Breach (Act 3)",shattered_core:"The Shattered Core"}[h]||"New zone"} unlocked!`)}const c=w("div","cbt-end-modal");c.innerHTML=`
      <div class="cem-box">
        ...
        <div class="cem-rewards">
          ...
          ${a.length?`<div class="cer" style="grid-column:1/-1"><span>Items Found</span><strong style="color:${$.magic}">${a.map(h=>h.name).join(", ")}</strong></div>`:""}
          ${o.length?`<div class="cer" style="grid-column:1/-1;color:#e8a020">⭐ ${o.map(h=>`${h.name} reached Level ${h.level}!`).join(" ")}</div>`:""}
          ${l?`<div class="cer" style="grid-column:1/-1;color:#60c0ff;font-weight:700">🗺 ${l}</div>`:""}
        </div>
        <button class="cem-btn">Continue</button>
      </div>
    `,c.querySelector(".cem-btn").addEventListener("click",()=>{this.audio.playSfx("click");const d=m.get().party.some(p=>(p.pendingAttrPoints||0)>0);this.manager.pop(),d&&this.manager.push(new ue(this.manager,this.audio))}),this._el.appendChild(c)}_defeat(){const e=m.get(),t=Math.floor(m.getGold()*.1);m.addGold(-t),e.party.forEach(s=>{s.hp=Math.max(1,Math.floor((s.maxHp||50)*.25))});const a=w("div","cbt-end-modal");a.innerHTML=`
      <div class="cem-box">
...
`,Ae=["weapon","offhand","head","chest","legs","hands","feet","ring1","ring2","necklace"],Ie={weapon:"Weapon",offhand:"Off-hand",head:"Head",chest:"Chest",legs:"Legs",hands:"Hands",feet:"Feet",ring1:"Ring",ring2:"Ring",necklace:"Necklace"};class ze{constructor(e,t){this.manager=e,this.audio=t,this._el=null,this._selectedCharIdx=0,this._tt=null}onEnter(){this._build()}_build(){E("inv-styles",qe),this._el=w("div","inv-screen"),this.manager.uiOverlay.appendChild(this._el),this._render()}_render(){const e=m.get(),t=[...e.party,...e.companions],a=t[this._selectedCharIdx]||t[0];this._selectedCharIdx>=t.length&&(this._selectedCharIdx=0),this._el.innerHTML=`
        ...
        <main class="town-main">
          <div class="town-header-row">
            ...
            <div class="gold-display" style="flex-direction:column;align-items:flex-end;gap:0.2rem">
              ...
              <div style="font-size:0.62rem;color:#8a7a6a">⭐ ${m.getFame()} Fame · ${m.getFameTitle()}</div>
            </div>
          </div>
...
`,Fe=["STR","DEX","INT","CON"],We=10,z=8;class Ue{constructor(e,t){this.manager=e,this.audio=t,this._el=null,this._name="Hero",this._class=null,this._attrs={STR:z,DEX:z,INT:z,CON:z},this._pointsSpent=0,this._step="class",this._saveSlot=0}get _pointsLeft(){return We-this._pointsSpent}onEnter(){this._build()}_build(){E("cb-styles",Ye),this._el=w("div","cb-screen"),this.manager.uiOverlay.appendChild(this._el),this._render()}_render(){this._el.innerHTML="",this._step==="class"?this._renderClassStep():this._renderStatsStep()}_renderClassStep(){var a;const e=this._el;e.innerHTML=`
      <div class="cb-header">
