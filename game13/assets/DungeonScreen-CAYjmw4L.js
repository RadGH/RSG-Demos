const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/play-BVrU3RoE.js","assets/modulepreload-polyfill-B5Qt9EMX.js","assets/savesClient-Dh5r3e2b.js","assets/version-DRUxWzGq.js","assets/play-BnSWl064.css"])))=>i.map(i=>d[i]);
import{D as b,i as m,r as c,c as l,E as h,C as f,G as o,j as _,_ as v}from"./play-BVrU3RoE.js";import"./modulepreload-polyfill-B5Qt9EMX.js";import"./savesClient-Dh5r3e2b.js";import"./version-DRUxWzGq.js";const y=`
...
`;class ${constructor(t,e,n,s,i){this.manager=t,this.audio=e,this._dungeonId=n,this._anchorNodeId=s,this._anchorZoneId=i,this._dungeon=b[n],this._stageIdx=0,this._el=null}onEnter(){if(m("dg-styles",y),!this._dungeon){console.warn("[DungeonScreen] unknown dungeonId",this._dungeonId),this.manager.pop();return}this._build()}onResume(){if(this._lastCombatVictory){if(this._lastCombatVictory=!1,this._stageIdx++,this._stageIdx>=this._dungeon.stages.length){this._showVictory();return}}else if(this._lastCombatDefeat){this._lastCombatDefeat=!1,this.manager.pop();return}this._build()}onLeave(){c(this._el),this._el=null}_build(){this._el&&c(this._el),this._el=l("div","dg-screen");const t=this._dungeon.stages[this._stageIdx],e=this._dungeon.stages.length,n=Array.from({length:e},(r,d)=>d<this._stageIdx?'<div class="pip done"></div>':d===this._stageIdx?'<div class="pip active"></div>':'<div class="pip"></div>').join(""),s=t.type,i=`Stage ${this._stageIdx+1} / ${e}`;let a="";if(s==="combat"||s==="boss"){const r=h[t.encounter],d=r?(r.enemies||[]).reduce((p,u)=>p+(u.count||1),0):0,g=s==="boss";a=`
        <div class="dg-card">
          <span class="dg-stage-tag" style="${g?"color:#c060e0;background:rgba(192,96,224,0.15)":""}">${i} · ${g?"Mini-Boss":"Combat"}</span>
        ...
        <div class="dg-card">
          <span class="dg-stage-tag">${i} · Discovery</span>
        ...
        <div class="dg-card">
          <span class="dg-stage-tag" style="color:#60c0e0;background:rgba(96,192,224,0.15)">${i} · Shrine</span>
      ...
      <div class="dg-progress">${n}</div>
      <div class="dg-body">
        ${a}
        <div class="dg-info">No retreat to town inside a dungeon — only "Give Up" returns you to the surface (and forfeits the chest).</div>
      </div>
    `,this.manager.uiOverlay.appendChild(this._el),this._wire()}_wire(){const t=this._el.querySelector("#dg-giveup");t&&t.addEventListener("click",()=>this._giveUp());const e=this._el.querySelector("#dg-next");e&&e.addEventListener("click",()=>this._advance());const n=this._el.querySelector("#dg-fight");n&&n.addEventListener("click",()=>this._engageCombat());const s=this._el.querySelector("#dg-rest");s&&s.addEventListener("click",()=>{this._restoreParty(),this._advance()});const i=this._el.querySelector("#dg-skip");i&&i.addEventListener("click",()=>this._advance())}_engageCombat(){var s,i;const t=this._dungeon.stages[this._stageIdx],e=h[t.encounter];if(!e){console.warn("[DungeonScreen] missing encounter",t.encounter),this._advance();return}const n={...e,name:t.name||e.name,_zoneId:this._anchorZoneId,_dungeonStage:this._stageIdx};(i=(s=this.audio)==null?void 0:s.playSfx)==null||i.call(s,"click"),this._lastCombatVictory=!1,this._lastCombatDefeat=!1,this._snapshotPartyHp(),this.manager.push(new f(this.manager,this.audio,null,n)),this._lastCombatVictory=!0}_snapshotPartyHp(){return(o.get().party||[]).map(e=>(e==null?void 0:e.hp)||0)}_restoreParty(){const t=o.get();[...t.party||[],...t.companions||[]].forEach(e=>{e&&(e.maxHp&&(e.hp=e.maxHp),e.maxMp&&(e.mp=e.maxMp))})}_advance(){var t,e;if((e=(t=this.audio)==null?void 0:t.playSfx)==null||e.call(t,"click"),this._stageIdx++,this._stageIdx>=this._dungeon.stages.length){this._showVictory();return}this._build()}_giveUp(){var e,n;(n=(e=this.audio)==null?void 0:e.playSfx)==null||n.call(e,"click");const t=o.get();if(this._anchorZoneId&&(t.zoneId=this._anchorZoneId),this._anchorNodeId){t.nodeId=this._anchorNodeId;try{o.setZoneNode(this._anchorZoneId,this._anchorNodeId)}catch{}}this.manager.pop()}_showVictory(){this._el&&c(this._el),this._el=l("div","dg-screen");const t=this._dungeon.reward||{},e=o.get();Array.isArray(e.completedDungeons)||(e.completedDungeons=[]),e.completedDungeons.includes(this._dungeon.id)||e.completedDungeons.push(this._dungeon.id),t.gold&&o.addGold(t.gold);let n=null;try{t.item&&(n=_(t.item,"rare","high"),n&&(o.addToInventory(n),v(()=>import("./play-BVrU3RoE.js").then(s=>s.s),__vite__mapDeps([0,1,2,3,4])).then(s=>s.recordDrop(n,{zoneId:this._anchorZoneId,source:"chest"})).catch(()=>{})))}catch(s){console.warn("[DungeonScreen] reward item gen failed",s)}if(this._el.innerHTML=`
      ...
      <div class="dg-body">
        <div class="dg-card">
          ...
          <p style="color:#e8d090">
            ...
            ${n?`· <span style="color:#60c0e0">${n.name}</span>`:""}
          </p>
          <div class="dg-buttons">
    ...
    `,this.manager.uiOverlay.appendChild(this._el),t.xp){const s=(e.party||[]).filter(i=>i&&!(i.isCompanion||i.class==="companion"));if(s.length){const i=Math.floor(t.xp/s.length);s.forEach(a=>{a.xp=(a.xp||0)+i})}}this._el.querySelector("#dg-leave").addEventListener("click",()=>this._giveUp())}}export{$ as DungeonScreen};
