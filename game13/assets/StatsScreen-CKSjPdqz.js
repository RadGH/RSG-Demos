import{e as M,i as E,c as j,r as I,g as D,a as B,p as P,b as R,d as H,A as w,f as K,G as F}from"./play-BbhfFBAv.js";import"./modulepreload-polyfill-B5Qt9EMX.js";import"./savesClient-Dh5r3e2b.js";import"./version-w0TUOj_k.js";function G(o,e,s={}){const t=o.getContext("2d"),a=window.devicePixelRatio||1,r=o.getBoundingClientRect();(o.width!==r.width*a||o.height!==r.height*a)&&(o.width=r.width*a,o.height=r.height*a),t.setTransform(a,0,0,a,0,0);const n=r.width,i=r.height;t.clearRect(0,0,n,i);const p=s.xKey||"t",x=s.yKey||"dps",m=s.color||"#e8a020",S="#5a4838",L="#8a7a6a",y=Object.assign({top:14,right:14,bottom:32,left:46},s.padding||{}),f=y.left,v=n-y.right,h=i-y.bottom,_=y.top;if(t.strokeStyle=S,t.lineWidth=1,t.beginPath(),t.moveTo(f,_),t.lineTo(f,h),t.lineTo(v,h),t.stroke(),!e||e.length===0){t.fillStyle=L,t.font="12px Inter, system-ui, sans-serif",t.textAlign="center",t.fillText(s.emptyLabel||"No data yet",(f+v)/2,(h+_)/2);return}let g=1/0,u=-1/0,$=0,b=-1/0;for(const l of e){const d=l[p],c=l[x];d<g&&(g=d),d>u&&(u=d),c>b&&(b=c)}u===g&&(u=g+1),b===0&&(b=1);const k=l=>f+(l-g)*(v-f)/(u-g),z=l=>h-(l-$)*(h-_)/(b-$);t.fillStyle=L,t.font="10px JetBrains Mono, monospace",t.textAlign="right",t.textBaseline="middle";const T=4;for(let l=0;l<=T;l++){const d=$+(b-$)*(l/T),c=z(d);t.strokeStyle=l===0?S:"rgba(90,72,56,0.45)",t.beginPath(),t.moveTo(f,c),t.lineTo(v,c),t.stroke(),t.fillText(N(d),f-6,c)}t.textAlign="center",t.textBaseline="top";for(let l=0;l<=3;l++){const d=g+(u-g)*(l/3),c=k(d);t.fillText(`${Math.round(d)}s`,c,h+6)}t.strokeStyle=m,t.lineWidth=2,t.beginPath();for(let l=0;l<e.length;l++){const d=k(e[l][p]),c=z(e[l][x]);l===0?t.moveTo(d,c):t.lineTo(d,c)}t.stroke(),t.lineTo(k(e[e.length-1][p]),h),t.lineTo(k(e[0][p]),h),t.closePath();const C=t.createLinearGradient(0,_,0,h);C.addColorStop(0,A(m,.3)),C.addColorStop(1,A(m,.02)),t.fillStyle=C,t.fill(),s.title&&(t.fillStyle="#e8a020",t.font="bold 12px Inter, system-ui, sans-serif",t.textAlign="left",t.textBaseline="top",t.fillText(s.title,f,0))}function N(o){return o>=1e3?(o/1e3).toFixed(1)+"k":Math.abs(o)<1?o.toFixed(2):Math.round(o).toString()}function A(o,e){const s=o.match(/^#([0-9a-f]{6})$/i);if(!s)return`rgba(232,160,32,${e})`;const t=parseInt(s[1],16);return`rgba(${t>>16&255},${t>>8&255},${t&255},${e})`}class V{constructor(e,s,t={}){this.manager=e,this.audio=s,this._tab=t.tab||"party",this._selectedCharId=null,this._filterLog="all",this._lifetimeOnly=!!t.lifetimeOnly}onEnter(){M(),E("stats-styles",W),this._el=j("div","stats-screen"),document.body.appendChild(this._el),this._lifetimeOnly&&(this._tab="lifetime"),this._render()}onExit(){I(this._el),this._el=null}destroy(){I(this._el),this._el=null}update(){}draw(){}_render(){const e=this._lifetimeOnly?[["lifetime","Lifetime"],["achievements","Achievements"]]:[["party","Party"],["lifetime","Lifetime"],["achievements","Achievements"]];this._el.innerHTML=`
      <div class="st-header">
    ...
    `,this._el.querySelector("#st-close").addEventListener("click",()=>{this.audio.playSfx("click"),this.manager.pop()}),this._el.querySelectorAll(".st-tab").forEach(s=>s.addEventListener("click",()=>{this.audio.playSfx("click"),this._tab=s.dataset.tab,this._render()})),this._postRender()}_renderTab(){return this._tab==="party"?this._renderParty():this._tab==="lifetime"?this._renderLifetime():this._tab==="achievements"?this._renderAchievements():""}_renderParty(){const e=F.get(),s=[...e.party||[],...e.companions||[]];if(!s.length)return'<div class="st-empty">No party members yet.</div>';(!this._selectedCharId||!s.find(i=>i.id===this._selectedCharId))&&(this._selectedCharId=s[0].id);const t=s.find(i=>i.id===this._selectedCharId),a=D(t.id),r=B(t.id),n=this._filterLog==="all"?r:r.filter(i=>i.type===this._filterLog);return`
      <div class="st-party-grid">
        <div class="st-roster">
          ${s.map(i=>`
            <button class="st-roster-row${i.id===this._selectedCharId?" active":""}" data-char="${i.id}">
              <span class="st-roster-portrait">${P(i,36,"st-portrait")}</span>
              <span class="st-roster-name">
                <span class="rn">${i.name}</span>
                <span class="rc">${i.cls||i.class||"companion"} · L${i.level||1}</span>
              </span>
              <span class="st-roster-kills">${D(i.id).kills}<span class="lk">k</span></span>
            </button>
          `).join("")}
        </div>
        <div class="st-detail">
    ...
    `}_statCell(e,s){return`<div class="st-cell"><div class="st-cell-label">${e}</div><div class="st-cell-value">${this._fmt(s)}</div></div>`}_fmt(e){return typeof e!="number"?String(e):e>=1e6?(e/1e6).toFixed(1)+"M":e>=1e3?(e/1e3).toFixed(1)+"k":Math.round(e).toString()}_logTagLabel(e){return e==="major_kill"?"Boss":e==="elite_kill"?"Elite":e==="near_death"?"Near Death":e==="death"?"Fell":e==="story"?"Story":e}_renderLifetime(){const e=R(),s=e.global||{},t=(e.runHistory||[]).slice(0,20);return`
    ...
    `}_renderAchievements(){const{current:e,life:s}=H(),t=w.length;return`
      <div class="st-ach">
        <div class="st-section-title">Achievements <span class="st-progress">${w.filter(r=>{var n,i;return((n=e[r.id])==null?void 0:n.unlocked)||((i=s[r.id])==null?void 0:i.unlocked)}).length} / ${t}</span></div>
        <div class="st-ach-grid">
          ${w.map(r=>{var x,m;const n=!!((x=s[r.id])!=null&&x.unlocked),i=!!((m=e[r.id])!=null&&m.unlocked),p=n||i;return`
