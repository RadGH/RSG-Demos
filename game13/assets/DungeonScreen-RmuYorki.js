const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/play-DM5eccWo.js","assets/savesClient-DUFjgBxb.js","assets/play-Dk4YEwwI.css"])))=>i.map(i=>d[i]);
import{af as L,i as N,G as r,r as m,c as x,ag as k,ah as S,a7 as U,a9 as P,_ as T,ai as V}from"./play-DM5eccWo.js";import"./savesClient-DUFjgBxb.js";const I=1.35,$=1.3,E=4,w=6,z=`
...
`;class R{constructor(e,t,s,i,a){this.manager=e,this.audio=t,this._dungeonId=s,this._anchorNodeId=i,this._anchorZoneId=a,this._dungeon=L[s],this._stageIdx=0,this._el=null,this._pendingStunBuff=!1,this._victoryPending=!1}onEnter(){if(N("dg-styles",z),!this._dungeon){console.warn("[DungeonScreen] unknown dungeonId",this._dungeonId),this.manager.pop();return}this._build()}onResume(){if(this._el&&(this._el.style.display=""),this._victoryPending){this._victoryPending=!1,this._showVictoryCard();return}if(this._nextStagePending){if(this._nextStagePending=!1,this._stageIdx++,this._stageIdx>=this._dungeon.stages.length){this._doVictorySequence();return}this._build();return}if(this._lastCombatVictory){this._lastCombatVictory=!1;const e=r.get();if([...e.party||[],...e.companions||[]].some(s=>s&&s._lastLevelUp)){this._nextStagePending=!0;return}if(this._stageIdx++,this._stageIdx>=this._dungeon.stages.length){this._doVictorySequence();return}}else if(this._lastCombatDefeat){this._lastCombatDefeat=!1,this.manager.pop();return}this._build()}onLeave(){m(this._el),this._el=null}onExit(){m(this._el),this._el=null}destroy(){m(this._el),this._el=null}_build(){if(this._el&&m(this._el),this._el=x("div","dg-screen"),this._stageIdx>=this._dungeon.stages.length){this._doVictorySequence();return}const e=this._dungeon.stages[this._stageIdx],t=this._dungeon.stages.length,s=Array.from({length:t},(o,d)=>d<this._stageIdx?'<div class="pip done"></div>':d===this._stageIdx?'<div class="pip active"></div>':'<div class="pip"></div>').join(""),i=e.type,a=`Stage ${this._stageIdx+1} / ${t}`;let n="";if(i==="combat"||i==="boss"){const o=k[e.encounter],g=this._scaledEncounterEnemies(o).reduce((h,u)=>h+(u.count||1),0),c=i==="boss",p=this._pendingStunBuff?'<div class="dg-buff-badge">Enemies will start stunned (skill check bonus)</div>':"";n=`
        <div class="dg-card">
          <span class="dg-stage-tag" style="${c?"color:#c060e0;background:rgba(192,96,224,0.15)":""}">${a} · ${c?"Mini-Boss":"Combat"}</span>
          <h2>${e.name||(o==null?void 0:o.name)||"Hostile encounter"}</h2>
          <p>${g} ${g===1?"enemy":"enemies"} ahead. ${c?"A mini-boss waits at the end of the run — defeat it to claim the chest.":"Cut through and press deeper."}</p>
          ${p}
          <div class="dg-buttons">
            <button type="button" class="dg-btn danger" id="dg-giveup">Give Up</button>
            <button type="button" class="dg-btn primary" id="dg-fight">${c?"Confront Boss":"Engage"}</button>
          </div>
    ...
    `,this.manager.uiOverlay.appendChild(this._el),this._el.querySelector("#dg-leave").addEventListener("click",()=>this._giveUp())}}export{R as DungeonScreen};
