import{D as o,U as v,b,S as u,W as y,E as x}from"./main-GcCELsWO.js";import h from"./CoopSystem-DmpFwNVw.js";import{W as w}from"./WorldGenerator-BYrz5_dP.js";import"./Noise-Cgm4OGP4.js";const f="spellroads-partybuilder-style";function C(){if(document.getElementById(f))return;const c=document.createElement("style");c.id=f,c.textContent=`
    /* ── PartyBuilder Panel ───────────────────────────────────────── */
    #panel-party-builder {
      position: fixed;
      inset: 0;
  ...
  `,document.head.appendChild(c)}const g=[{id:"standard",name:"Standard Start",description:"Begin at a small settlement in the midlands. Medium world, temperate climate.",params:{worldSize:"medium",climate:"temperate",magicDensity:"medium",monsterDensity:"medium"}},{id:"harsh_north",name:"Harsh North",description:"Start in the frozen north. Difficult terrain, stronger enemies, rare loot.",params:{worldSize:"medium",climate:"harsh",magicDensity:"low",monsterDensity:"high"}},{id:"magic_rich",name:"Magic-Rich Lands",description:"A world teeming with magical anomalies, rare trees, and legendary encounters.",params:{worldSize:"large",climate:"temperate",magicDensity:"high",monsterDensity:"medium"}},{id:"small_world",name:"Small World",description:"Compact map — faster completion, tighter encounters. Good for learning.",params:{worldSize:"small",climate:"temperate",magicDensity:"medium",monsterDensity:"medium"}},{id:"epic_saga",name:"Epic Saga",description:"A vast world with many factions, towns, and storylines. For experienced players.",params:{worldSize:"epic",climate:"temperate",magicDensity:"high",monsterDensity:"high"}}],S={soldier:["Iron Sword","Wooden Shield","Leather Armor"],mercenary:["Handaxe","Light Armor","50 Gold"],scholar:["Tome of Knowledge","Staff","Apprentice Robes"],healer:["Healing Potion x3","Wooden Staff","Healer's Satchel"],skald:["Skald's Lute","Dagger","Traveler's Cloak"],thief:["Dagger","Lockpick x3","Dark Cloak"],ranger:["Shortbow","Quiver (20 Arrows)","Hunting Knife"],sea_captain:["Captain's Cutlass","Navigation Charts","Sea Cloak"],runecarver:["Rune Scroll x2","Chisel","Scholar's Robes"],caravan_guard:["Spear","Horse Saddle","Wagon Deed"]};class P extends v{constructor(e,t){super("panel-party-builder",{trapFocus:!1,zLayer:"panel"}),this._heroes=e??[],this._onStart=t,this._partySlots=[...this._heroes,null,null,null,null].slice(0,4),this._hiredNPCs=[],this._gold=200,this._selectedScenario="standard",this._npcPool=[];for(let a=0;a<3;a++)this._npcPool.push(b.createNPC({level:1}))}build(){this.el.innerHTML="";const e=document.createElement("div");e.className="pb-root",e.setAttribute("role","dialog"),e.setAttribute("aria-label","Party Builder"),e.appendChild(this._buildHeader());const t=document.createElement("div");return t.className="pb-content",t.id="pb-content",this._colParty=this._buildPartyColumn(),this._colHire=this._buildHireColumn(),this._colScenario=this._buildScenarioColumn(),t.appendChild(this._colParty),t.appendChild(this._colHire),t.appendChild(this._colScenario),e.appendChild(t),e.appendChild(this._buildFooter()),this.el.appendChild(e),this}_buildHeader(){const e=document.createElement("div");return e.className="pb-header",e.innerHTML=`
    ...
    `,!e.isHero){const d=document.createElement("div");d.className="pb-char-controls";const s=document.createElement("button");s.type="button",s.className="pb-dismiss-btn",s.textContent="Dismiss",s.addEventListener("click",()=>{this._dismissNPC(e.id,t)}),d.appendChild(s),a.appendChild(d)}return a}_buildHireColumn(){const e=document.createElement("div");return e.className="pb-col",e.id="pb-col-hire",this._refreshHireColumn(e),e}_refreshHireColumn(e){if(e=e??document.getElementById("pb-col-hire"),!!e){if(e.innerHTML='<div class="pb-col-title">Tavern — Hire Companions</div>',this._npcPool.length===0){const t=document.createElement("p");t.style.cssText="font-size:0.75rem; color:#5a4a2a;",t.textContent="No more companions available in this town.",e.appendChild(t);return}if(this._npcPool.forEach(t=>{if(this._hiredNPCs.some(m=>m.id===t.id))return;const r=document.createElement("div");r.className="pb-char-card",r.style.marginBottom="10px";const n=b.getRace(t.race)?.name??t.race,l=t.wage*3,p=t.attributes,d=["might","agility","intellect","wisdom","endurance","presence"].map(m=>{const _=p[m]??10;return`<span class="pb-attr-pill">${m.slice(0,3).toUpperCase()} <strong>${_}</strong></span>`}).join("");r.innerHTML=`
      ...
      `;const s=document.createElement("button");s.type="button",s.className="pb-hire-btn",s.style.marginTop="8px",s.textContent=`Hire (${l} gold)`,s.disabled=this._gold<l||this._partySlots.filter(Boolean).length>=4,s.id=`pb-hire-btn-${t.id}`,s.addEventListener("click",()=>{this._hireNPC(t,l)}),r.appendChild(s),e.appendChild(r)}),this._npcPool.every(t=>this._hiredNPCs.some(a=>a.id===t.id))){const t=document.createElement("p");t.style.cssText="font-size:0.75rem; color:#5a4a2a;",t.textContent="All available companions hired.",e.appendChild(t)}}}_hireNPC(e,t){const a=this._partySlots.findIndex(r=>r===null);if(a===-1){o.warn("[party] Cannot hire NPC: party full");return}if(this._gold<t){o.warn("[party] Cannot hire NPC: insufficient gold");return}this._gold-=t,this._hiredNPCs.push(e),this._partySlots[a]=e,o.info(`[party] Hired NPC; name=${e.name} cost=${t} gold=${this._gold}`),this._updateGoldDisplay(),this._refreshPartyColumn(),this._refreshHireColumn(),this._updateStartButton()}_dismissNPC(e,t){const a=this._hiredNPCs.find(i=>i.id===e);if(!a)return;this._hiredNPCs=this._hiredNPCs.filter(i=>i.id!==e),this._partySlots[t]=null;const r=this._partySlots.filter(Boolean);this._partySlots=[...r,null,null,null,null].slice(0,4),o.info(`[party] Dismissed NPC; name=${a.name}`),this._refreshPartyColumn(),this._refreshHireColumn(),this._updateStartButton()}_buildScenarioColumn(){const e=document.createElement("div");e.className="pb-col",e.id="pb-col-scenario",e.innerHTML='<div class="pb-col-title">Starting Scenario</div>',g.forEach(r=>{const i=document.createElement("button");i.type="button",i.className="pb-scenario-card"+(this._selectedScenario===r.id?" selected":""),i.dataset.scenarioId=r.id,i.innerHTML=`
        <div class="pb-scenario-name">${r.name}</div>
        <div class="pb-scenario-desc">${r.description}</div>
      `,i.addEventListener("click",()=>{this._selectedScenario=r.id,e.querySelectorAll(".pb-scenario-card").forEach(n=>n.classList.remove("selected")),i.classList.add("selected"),this._refreshGearList(e)}),e.appendChild(i)});const t=document.createElement("div");t.className="pb-col-title",t.style.marginTop="16px",t.textContent="Starting Gear",t.id="pb-gear-title",e.appendChild(t);const a=document.createElement("div");return a.className="pb-gear-list",a.id="pb-gear-list",e.appendChild(a),this._refreshGearList(e),e}_refreshGearList(e){e=e??document.getElementById("pb-col-scenario");const t=e?.querySelector("#pb-gear-list")??document.getElementById("pb-gear-list");if(!t)return;t.innerHTML="";const a=this._partySlots.filter(r=>r&&r.isHero);if(a.length===0){t.innerHTML='<div class="pb-gear-item" style="color:#5a4a2a;">No heroes in party.</div>';return}a.forEach(r=>{const i=r.background?.adult??"soldier",n=S[i]??["Basic Sword","Travel Rations"],l=document.createElement("div");l.style.marginBottom="8px",l.innerHTML=`
        <div style="font-size:0.7rem; color:#8a7a50; margin-bottom:3px;">${r.name}</div>
        ${n.map(p=>`<div class="pb-gear-item">• ${p}</div>`).join("")}
      `,t.appendChild(l)})}_managePlayerAssignments(){if(this._partySlots.filter(Boolean).length<2){o.info("[party] Only one party member — P1 assignment unchanged");return}const t=this._partySlots[0];this._partySlots[0]=this._partySlots[1],this._partySlots[1]=t,this._refreshPartyColumn(),o.info("[party] Player assignments swapped")}_buildFooter(){const e=document.createElement("div");e.className="pb-footer";const t=document.createElement("button");return t.type="button",t.className="pb-btn",t.textContent="Back to Character Creation",t.addEventListener("click",()=>{o.info("[party] Back to charCreate"),u.transition("charCreate")}),this._statusMsg=document.createElement("span"),this._statusMsg.className="pb-status-msg",this._statusMsg.textContent="Choose a scenario and start your adventure.",this._startBtn=document.createElement("button"),this._startBtn.type="button",this._startBtn.className="pb-btn primary",this._startBtn.id="pb-btn-start",this._startBtn.textContent="Start Adventure",this._startBtn.addEventListener("click",()=>this._startAdventure()),this._updateStartButton(),e.appendChild(t),e.appendChild(this._statusMsg),e.appendChild(this._startBtn),e}_updateStartButton(){if(!this._startBtn)return;const e=this._partySlots.some(t=>t&&t.isHero);this._startBtn.disabled=!e}_startAdventure(){const e=this._partySlots.filter(Boolean);if(e.length===0){o.warn("[party] Cannot start — no party members");return}const t=g.find(n=>n.id===this._selectedScenario)??g[0],a=Date.now(),r={...t.params,seed:a,startingRegionLevel:1};o.info(`[party] Starting adventure; scenario=${t.id} seed=${a} party=${e.length}`);let i;try{i=w.generate(r)}catch(n){o.error(`[party] WorldGenerator failed: ${n.message}`);return}y.init(i),h.autoAssign(),y.playerCount=h.getPlayerCount(),window.__characterSystem=b,window.__coopSystem=h,x.emit("party:set",{party:e,gold:this._gold}),this._onStart&&this._onStart({party:e,worldData:i,gold:this._gold}),u.transition("localMap",{regionId:i.startRegionId})}destroy(){super.destroy()}}class k{constructor(){this._panel=null}async init({heroes:e=[]}={}){o.info("[boot] PartyBuilderScene init"),C(),window.__debug||(window.__debug={}),window.__debug.currentScene="partyBuilder",this._panel=new P(e,({party:t,worldData:a,gold:r})=>{o.info(`[party] Adventure started; partySize=${t.length} gold=${r}`)}),this._panel.build(),this._panel.show(),o.info("[boot] PartyBuilderScene ready")}update(e){}async destroy(){o.info("[boot] PartyBuilderScene destroy"),this._panel&&(this._panel.destroy(),this._panel=null),window.__debug&&window.__debug.currentScene==="partyBuilder"&&(window.__debug.currentScene=null)}}export{k as default};
//# sourceMappingURL=PartyBuilderScene-BJjRHoMN.js.map
