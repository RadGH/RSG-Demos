const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./main-C08doFuo.js","./main-D2c3OVlN.css"])))=>i.map(i=>d[i]);
import{U as Y,D as f,W as l,S as T,E as k,C as E,e as U,_ as G,c as B}from"./main-C08doFuo.js";import{s as F,n as W}from"./Noise-Cgm4OGP4.js";import{H}from"./HUDPanel-DvoCUcQL.js";import{G as y}from"./Graphics-_I3F_Jcl.js";import"./colorToUniform-DUza8Qlq.js";const D={might:"Might",agility:"Agility",wit:"Wit",presence:"Presence",endurance:"Endurance",attunement:"Attunement"},O={dialog:{label:"Encounter",color:"#c8a020"},combat:{label:"Ambush",color:"#c83020"},merchant:{label:"Traveler",color:"#4a9a4a"},shrine:{label:"Shrine",color:"#7a5aba"},discovery:{label:"Discovery",color:"#2a8aaa"},rest:{label:"Rest Site",color:"#4a7a4a"},trap:{label:"Danger",color:"#c83020"},default:{label:"Encounter",color:"#c8a020"}};class K extends Y{constructor(){super("panel-encounter-dialog",{trapFocus:!0,zLayer:"modal"}),this._encounter=null,this._partyChars=null,this._onComplete=null,this._inResultPhase=!1,this._chosenIndex=null,this._keyHandler=e=>this._onKeydown(e)}show(e,t,n){this._encounter=e,this._partyChars=t??[],this._onComplete=n??null,this._inResultPhase=!1,this._chosenIndex=null,this._renderEncounter(),this._addDarkOverlay(),window.addEventListener("keydown",this._keyHandler),super.show(),this._focusFirstOption(),f.event(`[encounter] Dialog shown; id=${e?.id??"unknown"} title="${e?.title??""}" options=${e?.options?.length??0}`)}hide(){window.removeEventListener("keydown",this._keyHandler),this._removeDarkOverlay(),super.hide(),this._encounter=null,this._partyChars=null,this._onComplete=null}_renderEncounter(){const e=this._encounter;if(!e)return;const t=O[e.type]??O.default,n=e.options??[],i=n.map((r,o)=>this._renderOption(r,o)).join("");this.el.innerHTML=`
...
`,A=new K,p=32,u=40,g=30,z=3,R=8,M=160,L=120,q=12,j={ocean:"water",frozen_sea:"water",tundra:"snow",taiga:"forest",forest:"forest",plains:"grass",desert:"sand",jungle:"forest",mountain:"mountain",volcanic:"mountain",archipelago:"sand"},X={grass:4880938,forest:2775578,mountain:8022618,water:1718876,sand:13150810,snow:15263984,road:9075290},Z={grass:660480,forest:528394,mountain:1051656,water:266272,sand:1576968,snow:526344,road:788488};class ie{async init({renderer:e,regionId:t}={}){if(this._renderer=e,this._regionId=t??l.currentRegionId??0,this._panel=null,this._tileContainer=null,this._fogTileContainer=null,this._partyContainer=null,this._minimapContainer=null,this._minimapGfx=null,this._camera={x:0,y:0},this._partyX=l.partyPosition.x,this._partyY=l.partyPosition.y,this._moveAccum={x:0,y:0},this._ticker=null,this._resizeUnsub=null,this._fogUpdateUnsub=null,this._fogDirty=!0,this._encounterActive=!1,this._distanceSinceCheck=0,this._encounterCooldown=0,this._prevTileX=null,this._prevTileY=null,window.__debug=window.__debug??{},window.__debug.currentScene="localMap",window.__debug.getLocalMap=()=>({regionId:this._regionId,gridW:u,gridH:g,partyX:Math.floor(this._partyX),partyY:Math.floor(this._partyY),tileCount:this._tiles?this._tiles.length:0}),!l.world){f.error("[world] LocalMapScene: no world data"),T.transition("worldMap");return}l.currentRegionId=this._regionId,l.currentLocalMapId=this._regionId,l.exploreRegion(this._regionId),this._tiles=this._getOrGenerateLocalMap(),this._buildPixiTiles(),this._buildPixiParty(),this._buildMinimap(),this._buildHTMLPanel(),window.__settings?.fogOfWar&&l.revealAround(Math.floor(this._partyX),Math.floor(this._partyY),R),this._fogDirty=!0,this._ticker=n=>this._update(n.deltaMS),this._renderer.app.ticker.add(this._ticker),this._resizeUnsub=k.on("renderer:resize",()=>this._onResize()),this._fogUpdateUnsub=k.on("fog:update",()=>{this._fogDirty=!0}),H.init(),k.emit("localmap:built",{regionId:this._regionId,gridW:u,gridH:g}),this._showOnboardingHint(),this._buildMobileDpad(),this._updatePartyPosition(),this._updateCamera(),window.__worldState=l,f.info(`[world] LocalMapScene initialized; regionId=${this._regionId} tiles=${this._tiles.length}`)}_getOrGenerateLocalMap(){const e=l.getLocalMap(this._regionId);if(e)return e.tiles;const t=l.world.regions.find(c=>c.id===this._regionId),n=t?.biome??"plains",i=j[n]??"grass",r=(l.world.seed^this._regionId*1514109713)>>>0;F(r);const o=B(r),s=[],a=.12;for(let c=0;c<g;c++)for(let d=0;d<u;d++){const m=W(d*a,c*a,4,.5,2);let h=i;i==="grass"?m>.72?h="forest":m<.18&&(h="water"):i==="forest"?m>.78?h="mountain":m<.15&&(h="grass"):i==="mountain"?m<.25?h="grass":m<.4&&(h="forest"):i==="snow"?m>.7&&(h="mountain"):i==="sand"&&(m<.15?h="water":m>.8&&(h="grass"));const _=X[h]??4473924,x=Z[h]??526344,v=o.nextInt(0,3);let w=_;if(v===0){const C=x>>16&255,b=x>>8&255,I=x&255,$=_>>16&255,P=_>>8&255,S=_&255;w=Math.min(255,$+C)<<16|Math.min(255,P+b)<<8|Math.min(255,S+I)}else if(v===1){const C=x>>16&255,b=x>>8&255,I=x&255,$=_>>16&255,P=_>>8&255,S=_&255;w=Math.max(0,$-C)<<16|Math.max(0,P-b)<<8|Math.max(0,S-I)}s.push({x:d,y:c,type:h,color:w,passable:h!=="water"&&h!=="mountain"})}return this._addRoadTiles(s,t),l.setLocalMap(this._regionId,{tiles:s,gridW:u,gridH:g}),s}_addRoadTiles(e,t){if(!t||!l.world.spellroads)return;const n=l.world,i=t.centerX/n.gridW,r=t.centerY/n.gridH;for(const o of n.spellroads)if(o.find(a=>{const c=a.x-i,d=a.y-r;return Math.sqrt(c*c+d*d)<.15})){for(let a=0;a<u;a++){const c=Math.floor(g/2)+Math.round(Math.sin(a*.3)*1.5);if(c>=0&&c<g){const d=e[c*u+a];d&&d.type!=="water"&&(d.type="road",d.color=X.road,d.passable=!0)}}break}}_buildPixiTiles(){const e=this._renderer.layers.world;this._tileContainer=new E,this._tileContainer.label="localTiles",e.addChild(this._tileContainer);const t=new Map;for(const n of this._tiles)t.has(n.color)||t.set(n.color,[]),t.get(n.color).push(n);for(const[n,i]of t){const r=new y;for(const o of i)r.rect(o.x*p,o.y*p,p,p);r.fill({color:n}),this._tileContainer.addChild(r)}this._tileContainer.addChild(new y),this._fogTileContainer=new E,this._fogTileContainer.label="localFog",e.addChild(this._fogTileContainer)}_rebuildFogTiles(){if(!this._fogTileContainer)return;if(!window.__settings?.fogOfWar){this._fogTileContainer.removeChildren(),this._fogDirty=!1;return}this._fogTileContainer.removeChildren();const e=new y,t=new y;for(let n=0;n<g;n++)for(let i=0;i<u;i++){const r=l.getFogAt(this._regionId,i,n);if(r===2)continue;const o=i*p,s=n*p;r===0?e.rect(o,s,p,p):t.rect(o,s,p,p)}e.fill({color:0,alpha:.9}),t.fill({color:0,alpha:.4}),this._fogTileContainer.addChild(e),this._fogTileContainer.addChild(t),this._fogDirty=!1}_buildPixiParty(){this._partyContainer=new E,this._partyContainer.label="partyToken",this._renderer.layers.entities.addChild(this._partyContainer),this._partyGfx=new y,this._partyContainer.addChild(this._partyGfx),this._drawPartyGfx()}_drawPartyGfx(){if(!this._partyGfx)return;this._partyGfx.clear();const e=0,t=0,n=10;this._partyGfx.circle(e,t,n+4),this._partyGfx.fill({color:16768256,alpha:.2}),this._partyGfx.circle(e,t,n),this._partyGfx.fill({color:16768256}),this._partyGfx.circle(e,t,n),this._partyGfx.stroke({color:16777215,width:2}),this._partyGfx.moveTo(e-6,t),this._partyGfx.lineTo(e+6,t),this._partyGfx.moveTo(e,t-6),this._partyGfx.lineTo(e,t+6),this._partyGfx.stroke({color:16777215,width:1.5})}_updatePartyPosition(){if(!this._partyContainer)return;const e=Math.floor(this._partyX)*p+p/2,t=Math.floor(this._partyY)*p+p/2;this._partyContainer.x=e,this._partyContainer.y=t}_updateCamera(){const e=this._renderer.width,t=this._renderer.height,n=-(Math.floor(this._partyX)*p+p/2-e/2),i=-(Math.floor(this._partyY)*p+p/2-t/2);this._camera.x=n,this._camera.y=i,this._tileContainer&&(this._tileContainer.x=this._camera.x,this._tileContainer.y=this._camera.y),this._fogTileContainer&&(this._fogTileContainer.x=this._camera.x,this._fogTileContainer.y=this._camera.y)}_onResize(){this._updateCamera(),this._updateMinimap(),this._repositionMinimap()}_update(e){const t=e/1e3;if(this._encounterActive)return;const n=U.getState(0),i=n.moveAxis.x,r=n.moveAxis.y;let o=!1;if(i!==0||r!==0){const s=Math.sqrt(i*i+r*r),a=i/s,c=r/s,d=a*z*t,m=c*z*t,h=this._partyX+d,_=this._partyY+m,x=Math.max(0,Math.min(u-1,h)),v=Math.max(0,Math.min(g-1,_)),w=Math.floor(x),C=Math.floor(v),b=this._tiles[C*u+w];b&&b.passable&&(this._partyX=x,this._partyY=v,o=!0)}if(o){const s=Math.floor(this._partyX),a=Math.floor(this._partyY);if(l.partyPosition.x=s,l.partyPosition.y=a,window.__settings?.fogOfWar&&l.revealAround(s,a,R),this._fogDirty=!0,k.emit("local:partyMoved",{x:s,y:a}),s!==this._prevTileX||a!==this._prevTileY){this._prevTileX=s,this._prevTileY=a;const d=this._tiles[a*u+s];if(d?.type==="dungeon"){this._promptDungeonEntry(d);return}this._advanceTimeForTile(s,a),this._distanceSinceCheck+=1,this._encounterCooldown>0&&(this._encounterCooldown-=1),this._distanceSinceCheck>=3&&this._encounterCooldown<=0&&(this._distanceSinceCheck=0,this._checkForEncounter(s,a))}}this._updatePartyPosition(),this._updateCamera(),this._fogDirty&&(this._rebuildFogTiles(),this._updateMinimap())}_checkForEncounter(e,t){const n=window.__encounterSystem??null;if(!n?.checkMovementEncounter)return;const i=l.getRegion(this._regionId),r=this._tiles[t*u+e],o={x:e,y:t,regionId:this._regionId,biome:i?.biome??"plains",tileType:r?.type??"grass",partyGold:l.partyGold,isNight:this._checkIsNight()};let s=null;try{s=n.checkMovementEncounter(o)}catch(a){f.warn(`[encounter] checkMovementEncounter failed: ${a.message}`);return}if(s)if(f.event(`[encounter] Encounter triggered; type=${s.type} id=${s.id??"unknown"} tile=${e},${t}`),this._encounterCooldown=8,s.type==="combat"){const a=window.__partyChars??[],c=s.enemies??[];T.transition("combat",{partyChars:a,enemies:c,sourceEncounter:s})}else this._showEncounterDialog(s)}_showEncounterDialog(e){this._encounterActive=!0;const t=window.__partyChars??[];A.show(e,t,n=>{this._encounterActive=!1,n&&(f.event(`[encounter] Dialog resolved; passed=${n.passed??!1} success=${n.success??"n/a"}`),n.goldDelta!=null&&(l.partyGold=Math.max(0,(l.partyGold??0)+n.goldDelta),H.update()),k.emit("encounter:resolved",{encounter:e,result:n}))})}_promptDungeonEntry(e){if(document.getElementById("dungeon-entry-prompt"))return;this._encounterActive=!0;const n=e.dungeonType??"burial_mound",i=e.dungeonName??n.replace(/_/g," ").replace(/\b\w/g,a=>a.toUpperCase()),r=((l.world?.seed??12345)^e.x*31295+e.y*7033)>>>0,o=l.currentRegionLevel??1,s=document.createElement("div");s.id="dungeon-entry-prompt",s.style.cssText=`
      position: fixed;
      top: 50%;
      left: 50%;
    ...
    `,document.body.appendChild(s),document.getElementById("btn-dungeon-yes")?.addEventListener("click",()=>{s.remove(),f.event(`[dungeon] Party entering dungeon; type=${n} seed=${r}`),T.transition("dungeon",{dungeonType:n,regionLevel:o,seed:r})}),document.getElementById("btn-dungeon-no")?.addEventListener("click",()=>{s.remove(),this._encounterActive=!1,f.info("[dungeon] Party declined dungeon entry")})}_advanceTimeForTile(e,t){const n=window.__timeSystem??null;if(!n?.advanceTime)return;const r=this._tiles[t*u+e]?.type??"grass";let o=1;try{n.getMoveCost&&(o=n.getMoveCost(r)),n.advanceTime(o)}catch(s){f.warn(`[encounter] TimeSystem.advanceTime failed: ${s.message}`)}}_checkIsNight(){try{if(window.__timeSystem?.isNight)return window.__timeSystem.isNight()}catch{}return!1}_buildMinimap(){const e=this._renderer.layers.hud;this._minimapContainer=new E,this._minimapContainer.label="minimapContainer",e.addChild(this._minimapContainer);const t=new y;t.rect(0,0,M,L),t.fill({color:0,alpha:.75}),t.rect(0,0,M,L),t.stroke({color:5917226,width:1}),this._minimapContainer.addChild(t),this._minimapGfx=new y,this._minimapContainer.addChild(this._minimapGfx),this._minimapPartyGfx=new y,this._minimapContainer.addChild(this._minimapPartyGfx),this._repositionMinimap(),this._updateMinimap()}_repositionMinimap(){if(!this._minimapContainer)return;const e=this._renderer.width,t=this._renderer.height;this._minimapContainer.x=e-M-q,this._minimapContainer.y=t-L-q}_updateMinimap(){if(!this._minimapGfx||!this._tiles)return;const e=M/u,t=L/g;this._minimapGfx.clear();for(const n of this._tiles){const i=l.getFogAt(this._regionId,n.x,n.y),r=n.x*e,o=n.y*t,s=Math.ceil(e),a=Math.ceil(t);if(i===0)this._minimapGfx.rect(r,o,s,a),this._minimapGfx.fill({color:0});else{this._minimapGfx.rect(r,o,s,a);const c=i===1?.5:1;this._minimapGfx.fill({color:n.color,alpha:c})}}if(this._minimapPartyGfx){this._minimapPartyGfx.clear();const n=Math.floor(this._partyX)*e+e/2,i=Math.floor(this._partyY)*t+t/2;this._minimapPartyGfx.circle(n,i,2.5),this._minimapPartyGfx.fill({color:16768256})}}_buildMobileDpad(){if(!("ontouchstart"in window)&&!navigator.maxTouchPoints)return;const e=document.getElementById("local-map-dpad");e&&e.remove();const t=document.createElement("div");t.id="local-map-dpad",t.style.cssText=`
      position: fixed;
      bottom: 20px;
      left: 20px;
      width: 120px;
    ...
    `;const n=[{id:"up",label:"▲",top:"0",left:"40px",dx:0,dy:-1},{id:"down",label:"▼",top:"80px",left:"40px",dx:0,dy:1},{id:"left",label:"◀",top:"40px",left:"0",dx:-1,dy:0},{id:"right",label:"▶",top:"40px",left:"80px",dx:1,dy:0}];for(const i of n){const r=document.createElement("button");r.textContent=i.label,r.style.cssText=`
      ...
      `,r.addEventListener("pointerdown",o=>{o.preventDefault(),G(()=>import("./main-C08doFuo.js").then(s=>s.aL),__vite__mapDeps([0,1]),import.meta.url).then(s=>{s.default._states[0].moveAxis.x=i.dx,s.default._states[0].moveAxis.y=i.dy})}),r.addEventListener("pointerup",()=>{G(()=>import("./main-C08doFuo.js").then(o=>o.aL),__vite__mapDeps([0,1]),import.meta.url).then(o=>{o.default._states[0].moveAxis.x=0,o.default._states[0].moveAxis.y=0})}),r.addEventListener("pointerleave",()=>{G(()=>import("./main-C08doFuo.js").then(o=>o.aL),__vite__mapDeps([0,1]),import.meta.url).then(o=>{o.default._states[0].moveAxis.x=0,o.default._states[0].moveAxis.y=0})}),t.appendChild(r)}document.body.appendChild(t),this._dpadEl=t}_buildHTMLPanel(){const e=document.getElementById("panel-local-map-hud");e&&e.remove();const t=document.createElement("div");t.id="panel-local-map-hud",t.style.cssText=`
      position: fixed;
      top: 12px;
      right: 12px;
    ...
    `;const i=l.world?.regions.find(r=>r.id===this._regionId);t.innerHTML=`
      ...
      ">World Map</button>
    `,document.body.appendChild(t),this._panel=t,document.getElementById("btn-local-world-map")?.addEventListener("click",()=>{T.transition("worldMap")})}_removeHTMLPanel(){this._panel&&(this._panel.remove(),this._panel=null)}_showOnboardingHint(){const e=document.getElementById("local-map-onboarding-hint");e&&e.remove();const t=document.createElement("div");t.id="local-map-onboarding-hint",t.style.cssText=`
      position: fixed;
